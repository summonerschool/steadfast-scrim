...
export class DiscordServiceImpl implements DiscordService {
  ...
  async createVoiceChannels(scrim: Scrim, blue: Team, red: Team): Promise<[VoiceChannel, VoiceChannel]> {
    const guild = await this.discordClient.guilds.fetch({ guild: scrim.guildID });
  ...
  async sendMatchDirectMessage(userIDs: string[], message: MessageCreateOptions): Promise<number> {
    const users = await Promise.all(userIDs.map((id) => this.discordClient.users.fetch(id)));
    const sent = await Promise.allSettled(users.map((u) => u.send(message)));
    return sent.filter((p) => p.status === 'fulfilled').length;
  }

  async deleteVoiceChannels(guildID: string, ids: string[]): Promise<[string, string] | null> {
    if (ids.length === 0) {
      return null;
    }
    const guild = await this.discordClient.guilds.fetch({ guild: guildID });
  ...
  async sendMessageInChannel(message: MessageCreateOptions): Promise<void> {
    let channel = this.discordClient.channels.cache.get(commandChannelID);
    if (!channel) {
      const res = await this.discordClient.channels.fetch(commandChannelID);
      if (res) channel = res;
      else {
  ...
  async createPostDiscussionThread(matchId: number, winner: GameSide, teamNames: [string, string]): Promise<string> {
    let channel = this.discordClient.channels.cache.get(feedbackChannelID);
    if (!channel) {
      const res = await this.discordClient.channels.fetch(feedbackChannelID);
      if (res) channel = res;
      else {
        throw new Error('Could not send message');
      }
