...
export interface MatchmakingService {
  ...
  attemptFill(queuers: User[], queuedFill?: string[]): { users: User[]; fillers: string[] };
}

export class MatchmakingServiceImpl implements MatchmakingService {
  startMatchmaking(users: User[], fillers = []): [Matchup, Matchup] {
    const playerPool = this.calculatePlayerPool(users, fillers);
    const combinations = this.generateAllPossibleTeams(playerPool);
    // team vs team with elo difference. The players are sorted by their ID within the team
    const res = this.findBestMatchup(combinations, users);
    if (!res.valid) {
  ...
  private calculatePlayerPool(users: User[], fillers: string[]) {
  ...
  private generateAllPossibleTeams(pool: User[][]) {
